...
def build_eval_indices(dataset_len: int, start_idx: int, num_images: int) -> list[int]:
    """Return contiguous indices for export."""
    if dataset_len <= 0:
        return []
...
def predictions_from_batch(
    ...
    for b in range(batch_size):
        ...
        for score, query_idx in zip(selected_scores, selected_indices):
            ...
            if valid_cat_ids is not None and coco_category_id not in valid_cat_ids:
                continue
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export COCO test2017 predictions JSON for official test-dev evaluation."
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to COCO test2017 image folder.",
    )
    parser.add_argument(
        "--test-ann-file",
        type=str,
        default=None,
        help="Path to COCO image info json (for example image_info_test-dev2017.json).",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Export batch size override (default: validation_monitor.batch_size).",
    )
    parser.add_argument(
        "--num-workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of images to export. <=0 means full test set.",
    )
    parser.add_argument(
        "--score-threshold",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not enforce category IDs from test annotation JSON.",
    )
    parser.add_argument(
        "--save-predictions",
        type=str,
        default="outputs/coco_test2017_predictions.json",
...
def main():
    # See lauch.json for an example of how to run this script with command-line arguments.
    args = parse_args()

    config = load_config(args.config)
    data_cfg = config.get("data", {})

    test_data_dir = args.test_data_dir or data_cfg.get("test_data_dir", None)
    test_ann_file = args.test_ann_file or data_cfg.get("test_ann_file", None)

    if not test_data_dir or not test_ann_file:
        raise ValueError(
            "Missing test paths. Provide --test-data-dir and --test-ann-file, "
            "or set data.test_data_dir and data.test_ann_file in config."
        )
    ...
    batch_size = int(args.batch_size or config.get("validation_monitor", {}).get("batch_size", 8))
    num_workers = int(args.num_workers if args.num_workers is not None else data_cfg.get("num_workers", 0))

    print("\n==== COCO test2017 export ====")
    print(f"  config: {args.config}")
    print(f"  checkpoint: {checkpoint_path}")
    print(f"  device: {device}")
    print(f"  test_data_dir: {test_data_dir}")
    print(f"  test_ann_file: {test_ann_file}")
    ...
    export_indices = build_eval_indices(
        dataset_len=len(dataset_full),
        start_idx=args.start_idx,
        num_images=args.num_images,
    )
    if not export_indices:
        raise RuntimeError("No images selected for export. Check --start-idx and --num-images.")

    dataset_export = (
        dataset_full if len(export_indices) == len(dataset_full) else Subset(dataset_full, export_indices)
    )
    ...
    coco_test = COCO(str(test_ann_file))
    valid_cat_ids = None
    if not args.disable_category_filter:
        cat_ids = {int(cat_id) for cat_id in coco_test.getCatIds()}
        if cat_ids:
            valid_cat_ids = cat_ids
    ...
    print("\nSaved test predictions JSON:")
    print(f"  {save_path}")
    print("Upload this file to COCO test-dev for official hidden-label metrics.")
